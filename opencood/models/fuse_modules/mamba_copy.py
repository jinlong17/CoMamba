...
class CoVSSBlock(nn.Module):
    def __init__(self, hidden_dim=256,patch_size=1, **kwargs):
        super().__init__()
        ...
        self.patch_embed = _make_patch_embed(in_chans, hidden_dim, patch_size, patch_norm, norm_layer=nn.LayerNorm, channel_first=False)
...
class MambaFusionEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.multi_scale =True #False #True
        ...
        for i in range(self.depth):
            ...
            block = CoVSSBlock(hidden_dim=256,patch_size=1)
            self.layers.append(block)

            if self.multi_scale:

                block_final = CoVSSBlock(hidden_dim=256,patch_size=2)
            else:
                block_final = CoVSSBlock(hidden_dim=256,patch_size=1)

            self.layers.append(block_final)       
        ...
        # ###----------->   1.   b m d h w -> (b m) d h w
        # self.pre_mm = nn.Sequential(
        #     Rearrange('b m d h w -> (b m) d h w')
        # )

        # self.post_mm = nn.Sequential(
        #     Rearrange('(b m) d h w -> b m d h w', m=5)
        # )

        # self.medium_mm = nn.Sequential(
        #     Rearrange('n h w b -> n b h w')
        # )

        ###----------->   2.   b m d h w -> b  d m (h w)
        self.pre_mm = nn.Sequential(
            Rearrange('b m d h w -> b d m (h w)')
        )

        self.post_mm = nn.Sequential(
            Rearrange('b d m (h w) -> b m d h w', h=48)
        )
        ...
        self.pre_upsample = nn.Sequential(
            Rearrange('b m d h w -> (b m) h w d')
        )

        self.post_upsample = nn.Sequential(
            Rearrange('(b m) h w d -> b m d h w', m=1)
        )

        #################-----------------> for feature fusion
        self.pre_fuse = nn.Sequential(
            Rearrange('n c s l -> n s c l') # bs, channel, cavs, h*w   ----->  bs,  cavs,  channel, h*w
        )

        self.post_fuse = nn.Sequential(
            Rearrange('n s c l -> n c s l')
        )        
        ...
        # groups = 1 #1        # Grouping for grouped convolution
        # radix = 16         # Radix for split attention
        # reduction_factor = 4  # Reduction factor for calculating inter_channels

        # # Initialize the SplAtConv2d module
        # self.sp_atten_n = SplAtConv2d(in_channels, channels, kernel_size, stride, padding,
        #                         dilation, groups, bias=True, radix=radix, 
        #                         reduction_factor=reduction_factor, norm_layer=nn.BatchNorm2d)
        # # in_channels = 2
        # # radix = 5
        # self.sp_atten_m = SplAtConv2d(in_channels, channels, kernel_size, stride, padding,
        #                         dilation, groups, bias=True, radix=radix, 
        #                         reduction_factor=reduction_factor, norm_layer=nn.BatchNorm2d)
        

        from .splat import VSS_SplAtConv2d
        ...
        # dilation = (1, 1)  # Dilation for the convolution
        # groups = 1 #1        # Grouping for grouped convolution
        # radix = 5         # Radix for split attention
        # reduction_factor = 4  # Reduction factor for calculating inter_channels

        # # Initialize the SplAtConv2d module
        # vss_block_n = CoVSSBlock(hidden_dim=5,patch_size=1)
        # self.sp_atten_n = VSS_SplAtConv2d(in_channels, channels, kernel_size, stride, padding,
        #                        dilation, groups, bias=True, radix=radix, 
        #                        reduction_factor=reduction_factor, norm_layer=nn.BatchNorm2d, vss_block=vss_block_n)
        # in_channels = 2
        # radix = 2
        # channels = 1
        # vss_block_m = CoVSSBlock(hidden_dim=2,patch_size=1)
        # self.sp_atten_m = VSS_SplAtConv2d(in_channels, channels, kernel_size, stride, padding,
        #                        dilation, groups, bias=True, radix=radix, 
        #                        reduction_factor=reduction_factor, norm_layer=nn.BatchNorm2d, vss_block=vss_block_m)
        ...
        padding = (1, 1)  # Padding for the convolution
        dilation = (1, 1)  # Dilation for the convolution
        groups = 8 #1        # Grouping for grouped convolution
        radix = 8         # Radix for split attention
        reduction_factor = 4  # Reduction factor for calculating inter_channels

        # Initialize the SplAtConv2d module
        vss_block_n = CoVSSBlock(hidden_dim=in_channels,patch_size=1)
        self.sp_atten_n = VSS_SplAtConv2d(in_channels, channels, kernel_size, stride, padding,
                               dilation, groups, bias=True, radix=radix, 
                               reduction_factor=reduction_factor, norm_layer=nn.BatchNorm2d, vss_block=vss_block_n)
        ...
        # #TODO: simple V2X fusion
        # self.simple_fusion = nn.Sequential(
        #     Reduce('b m d h w -> b d h w', 'mean')
        # )


    # def forward(self, x, mask=None):### 1) the baseline pipeline
    #     x = self.pre_mm(x)
    ...
    def vss_upsample(self, x):
        ...
        x = self.linear(x.view(B, H * W, C)).view(B, H, W, C).permute(0, 3, 1, 2).contiguous()  # B, C, H, W
        x = F.interpolate(x, scale_factor=(1,2), mode='bilinear', align_corners=False).permute(0, 2, 3, 1).contiguous()  # B, 2H, 2W, C
        x = self.norm(x)
        return x
    ...
    def forward(self, x, mask=None):### 1) the baseline pipeline
        ...
        # 2) V2X-VSS module fusion
        
        # pdb.set_trace()
            
        normal_feature = multi_feature[0]
        medium_feature = multi_feature[1]



        #--------> 1)
        # normal_feature = normal_feature.mean(dim=2, keepdim=True)
        # medium_feature = medium_feature.mean(dim=2, keepdim=True)

        #---------> 2)
        normal_feature = self.pre_fuse(normal_feature)
        medium_feature = self.pre_fuse(medium_feature)

        # pdb.set_trace()
        normal_feature = self.sp_atten_n(normal_feature)
        medium_feature = self.sp_atten_m(medium_feature)

        normal_feature = self.post_fuse(normal_feature)
        medium_feature = self.post_fuse(medium_feature)    
        ...
        if self.multi_scale:
            ...
            # pdb.set_trace()
            medium_feature = self.post_mm(medium_feature)
            medium_feature = self.pre_upsample(medium_feature)

            ######## upsample
            medium_feature = self.vss_upsample(medium_feature)  #--->[B, 2H, 2W, C]
  
            medium_feature = self.post_upsample(medium_feature)

            medium_feature = self.pre_mm(medium_feature)

        fusion_feature = torch.cat((normal_feature, medium_feature), dim=2)


            
        
        
        x = self.post_mm(fusion_feature)
        return self.mlp_head(x)
...
#         x = self.linear(x).permute(0, 3, 1, 2).contiguous()  # Shape: B, C/2, H, W
        
#         # Interpolate to double the width only
#         x = F.interpolate(x, scale_factor=(1, 2), mode='bilinear', align_corners=False).permute(0, 2, 3, 1).contiguous()  # Shape: B, H, 2W, C/2
...
class UpsampleExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        B, H, W, C = x.shape
        x = self.linear(x.view(B, H * W, C)).view(B, H, W, C).permute(0, 3, 1, 2).contiguous()  # B, C/2, H, W
...
if __name__ == "__main__":
    ...
    # model = VSSM().cuda() 
    # print("模型初始化成功")
    # # 创建一个假的输入张量来测试模型的前向传播
    # fake_input = torch.randn(1, 3, 224, 224).cuda()  # 假设输入尺寸为224x224，有3个颜色通道
    # output = model(fake_input)
    # print("前向传播输出尺寸:", output.size())
    ...
    # # 初始化模型
    # model = Backbone_VSSM(num_classes=1000, depths=[2, 2, 6, 2], dims=[64, 128, 256, 512], patch_size=4, in_chans=3)
    # model = model.to(device)

    # # 创建一个假的输入张量来测试模型的前向传播
    # fake_input = torch.randn(1, 3, 224, 224, device=device)  # 假设输入尺寸为224x224，有3个颜色通道

    # # 执行前向传播
    # output = model(fake_input)
    # print("输出尺寸:", [o.size() for o in output])
    ...
    input_tensor = torch.randn(1, 256, 48, 176).cuda()  # 假设输入是一个小批量的特征图
    ...
    # Example usage:
    # Assuming input tensor shape [4, 256, 1, 4224]
    # input_tensor = torch.randn(4, 256, 1, 4224)
    # model = UpsampleExpand(input_resolution=(1, 4224), dim=256)
    # output_tensor = model(input_tensor)
    # print("Output shape:", output_tensor.shape)  # Expected Output shape: [4, 256, 1, 8448]

    # # Create a tensor of shape (batch_size, height, width, channels)
    # batch_size, height, width, channels = 1, 10, 10, 256
    # input_tensor = torch.randn(batch_size, height, width, channels)
    
    # # Define module dimensions and instantiate it
    # dim = channels  # Same as the number of input channels
    # module = UpsampleExpand(input_resolution=(height, width), dim=dim)
    
    # # Forward pass of the tensor through the UpsampleExpand module
    # output_tensor = module(input_tensor)
    
    # # Print output shape
    # expected_height, expected_width = height * 2, width * 2
    # print("Output tensor shape:", output_tensor.shape)
    # print("Expected shape: [1, {}, {}, {}]".format(expected_height, expected_width, dim // 2))