...
class SS2Dv2:
    ...
    def forward_corev2(
        ...
        #####TODO:jinlong CrossScan_Ab_1direction, CrossMerge_Ab_1direction
...
class CoVSSBlock(nn.Module):
    def __init__(self, in_chans=256, hidden_dim=256,patch_size=1, **kwargs):
        super().__init__()
...
class MambaFusionEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i in range(self.depth):
            ...
            block = CoVSSBlock(in_chans=256, hidden_dim=256,patch_size=1)
            self.layers.append(block)

            if self.multi_scale:
                block_final = CoVSSBlock(in_chans=256,hidden_dim=512,patch_size=2)
            else:
                block_final = CoVSSBlock(in_chans=256,hidden_dim=256,patch_size=1)
        ...
        self.first_mediun_mm = nn.Sequential(
            Rearrange('(b m) h w c -> b c m (h w)', m=5)
        )
        ...
        self.medium_mm = nn.Sequential(
            Rearrange('n h w d -> n d h w')
        )

        self.multi_mm = nn.Sequential(
            Rearrange('n d m (h w) -> (n m) d h w', h=48)
        )
        self.multi_post = nn.Sequential(
            Rearrange('(n m) h w d -> n m (h w) d', m=5)
        )
        ...
        self.pre_fuse = nn.Sequential(
            Rearrange('n c s (h w) -> (n s) c h w', h=48) # [bs, channel, cavs, h*w]   ----->  [bs *cavs channel, h, w]
        )
        self.mul_pre_fuse = nn.Sequential(
            Rearrange('n c s (h w) -> (n s) c h w', h=24) # h/2 , w/2  [bs, channel, cavs, h*w]   ----->  [bs *cavs channel, h, w]
        )
        ...
        self.cov_upsample = nn.Sequential(
        ...
        groups = 4 #16 #1        # Grouping for grouped convolution
        radix = 4 #16         # Radix for split attention
        reduction_factor = 4  # Reduction factor for calculating inter_channels

        if self.v2x_vss_fusion:
            # Initialize the SplAtConv2d module
            vss_block_n = CoVSSBlock(in_chans=in_channels,hidden_dim=channels,patch_size=1)
            self.fusion_n = VSS_SplAtConv2d(in_channels, channels, kernel_size, stride, padding,
                                    dilation, groups, bias=True, radix=radix, 
                                    reduction_factor=reduction_factor, norm_layer=nn.BatchNorm2d,vss_block=vss_block_n)
            in_channels = 512
            channels = 512
            vss_block_m = CoVSSBlock(in_chans=in_channels,hidden_dim=channels,patch_size=1)
            self.fusion_m = VSS_SplAtConv2d(in_channels, channels, kernel_size, stride, padding,
                                    dilation, groups, bias=True, radix=radix, 
                                    reduction_factor=reduction_factor, norm_layer=nn.BatchNorm2d,vss_block=vss_block_m)
        else:

            self.fusion_n =  nn.Sequential(
            Rearrange('(b n) c h w -> b n c h w', n=5),
    ...
    def forward(self, x, mask=None):# 1) the baseline pipeline
        ...
        # x = self.pre_mm(x)  # x [bs, cavs, c, w-48, h-176] -------> x [bs, c, cavs, w-48 * h-176]   

        x = self.pre_mm(x)  # x [bs, cavs, c, w-48, h-176] -------> x [bs * cavs, c, w-48, h-176] 
        ...
        # the first 
        x = self.layers[0](x)  #-------> x [bs * cavs, w-48, h-176, c]  
        x = self.first_mediun_mm(x)  #-------> x [bs, c, cavs, w-48 * h-176]  
        multi_feature.append(x)

        # the second 
        if self.multi_scale:
            x = self.multi_mm(x)   #-------> x [bs * cavs, c , w-48, h-176]
            x = self.layers[1](x)  #-------> x [bs * cavs , w-48/2, h-176/2, c]
            x = self.multi_post(x) #-------> x [bs , cavs, w-48 * h-176, c]
        else:
            x = self.layers[1](x)
        
        x = self.medium_mm(x)   #-------> x [bs, c, cavs, w-48/2 * h-176/2] 
        multi_feature.append(x)
        ...
        if self.multi_scale:
            medium_feature = self.mul_pre_fuse(medium_feature)
        else:
            medium_feature = self.pre_fuse(medium_feature)

        if self.v2x_vss_fusion:
            normal_feature = self.fusion_n(normal_feature)
            medium_feature = self.fusion_m(medium_feature)
        else:
            # pdb.set_trace()
            normal_feature = self.fusion_n(normal_feature)
            medium_feature = self.fusion_n(medium_feature)
        ...
        if self.multi_scale:
            ...
            medium_feature = self.cov_upsample(medium_feature)  #   2C -----> C
