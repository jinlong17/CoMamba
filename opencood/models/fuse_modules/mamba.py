...
class MambaFusionEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i in range(self.depth):
            ...
            if self.multi_scale:
                ...
                block_final = CoVSSBlock(in_chans=1280,hidden_dim=2560,patch_size=2)#   5*256 =1280
            else:
        ...
        self.multi_mm = nn.Sequential(
            Rearrange('n d m (h w) -> n (d m) h w', h=48)
        )
        self.multi_post = nn.Sequential(
            Rearrange('n h w (d m) -> n m (h w) d', m=5)
        )
    ...
    def forward(self, x, mask=None):# 1) the baseline pipeline
        ...
        # the second 
        if self.multi_scale:
            x = self.multi_mm(x)   #-------> x [bs, c * cavs , w-48, h-176]
            x = self.layers[1](x)  #-------> x [bs , w-48/2, h-176/2, c * cavs]
